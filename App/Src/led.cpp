...
ros::ServiceServer<runbot_stm::ledRequest, runbot_stm::ledResponse> rgb_service(
		"stm/led", &color_cb);
...
uint32_t LEDManagerTaskCreate(ros::NodeHandle *nh) {
	nh_ = nh;
	nh_->advertiseService<runbot_stm::ledRequest, runbot_stm::ledResponse>(
			rgb_service);

	osThreadId_t LEDManagerHandle;
	const osThreadAttr_t led_control_attributes =
			{ name : "led_control", .attr_bits = osThreadDetached, .cb_mem =
					NULL, .cb_size = 0, .stack_mem = NULL,
					.stack_size = 128 * 4, .priority =
							(osPriority_t) osPriorityHigh, .tz_module = 0,
					.reserved = 0 };

	LEDManagerHandle = osThreadNew(ledControlTask, NULL,
			&led_control_attributes);

	if (NULL == LEDManagerHandle) {
		return 1;
	}
	return 0;
